...
(async () => {
	...
	for (const page of databaseQuery.results) {
		...
		const title = pageInfo.properties.Title.type === 'title' ? pageInfo.properties.Title.title[0]?.plain_text.trim() : 'Untitled';
		const slug =
			pageInfo.properties.Slug.type === 'rich_text'
				? pageInfo.properties.Slug.rich_text[0]?.plain_text.trim().toLowerCase().replace(/\s+/g, '-')
				: '';
		const description =
			pageInfo.properties.Description.type === 'rich_text' ? pageInfo.properties.Description.rich_text[0]?.plain_text.trim() : '';
