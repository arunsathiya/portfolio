...
I recently learned about Whoogle though and I had been wanting to try that for a while. It's a self-hosted, ad and tracker-free search engine that fetches results from Google. The project promises that it's free of cookie and IP address tracking too. It's open source and it seems that it can be set up on any device. I have two Raspberry Pi devices at home, both connected on my Tailscale network, [acting as Pi-hole nodes to block ads](https://blogarunsathiya.wordpress.com/2021/03/30/pi-hole-on-tailscale/). One of the two Raspberry Pis [also acts as my Hydroxide node to fetch ProtonMail emails](https://blogarunsathiya.wordpress.com/2021/07/29/hydroxide-headless-bridge-for-protonmail-on-tailscale/).
...
It's nice overall. I am not bothered by the `http` queries as [the communication between my device and the Raspberry Pi is end-to-end encrypted](https://tailscale.com/kb/1093/can-tailscale-decrypt-my-traffic/), thanks to [Tailscale](https://blogarunsathiya.wordpress.com/tag/tailscale/).
