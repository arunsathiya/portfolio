...
function isAllowedUserAgent(userAgent) {
	return userAgent && WP_USER_AGENT_PATTERNS.some((pattern) => userAgent.toLowerCase().includes(pattern.toLowerCase()));
}
...
export async function GET(context) {
	...
	return rss({
		...
		items: posts.map((post) => ({
			...
			link: `/${post.slug}/`,
			description:
				post.data.tags?.includes('Asides') || isAllowedUserAgent(userAgent)
